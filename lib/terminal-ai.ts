...
export async function callGateway(
  ...
  if (res.status === 401) {
    throw Object.assign(new Error('Session expired. Retry in a moment.'), { code: 'TOKEN_EXPIRED', retryable: true })
  }
  if (res.status === 402) {
    const body = await res.json().catch(() => ({})) as { redirect?: string }
    throw Object.assign(new Error('Insufficient credits. Please top up to continue.'), { code: 'INSUFFICIENT_CREDITS', redirect: body.redirect ?? '/pricing', retryable: false })
  }
  if (!res.ok) {
