...
import { getScreener, getSchemeById } from "../lib/mock";
import type { ScreenerRow, AssetClass } from "../lib/mock";
...
type SortKey = "scheme_name" | "category" | "aum" | "expense_ratio" | "holdings_count" | "deployable_cash" | "top10_concentration";
type Dir = "asc" | "desc";
...
export function ScreenView() {
  const { selectScheme } = useOutletContext<FundCtx>();
  ...
  const all = getScreener();

  const rows = useMemo(() => {
    const out = all.filter((r) => filter === "all" || r.asset_class === filter);
    out.sort((a, b) => {
      const av = a[sortKey];
      const bv = b[sortKey];
      let cmp: number;
  ...
  const maxCash = Math.max(...all.map((r) => r.deployable_cash));
  const maxConc = Math.max(...all.map((r) => r.top10_concentration));
      ...
      <div className="mt-4 bg-card border border-line-subtle rounded-sm overflow-hidden">
        <div className="overflow-x-auto scroll-thin">
          <table className="w-full border-collapse min-w-[860px]">
              ...
              <tr className="bg-subtle">
                {COLS.map((c) => {
                  ...
                  return (
                    <th
                      key={c.key}
                      className={["px-3 py-2.5 font-mono text-[10px] tracking-meta uppercase", c.align === "right" ? "text-right" : "text-left"].join(" ")}
                    >
                      <button
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="group border-b border-line-subtle last:border-0 hover:bg-subtle transition-colors">
                  <td className="px-3 py-3">
                    <div className="flex items-center gap-2.5">
                      <span className="h-2.5 w-2.5 rounded-sm shrink-0" style={{ background: ASSET_COLOR[r.asset_class] }} title={ASSET_LABEL[r.asset_class]} />
                  ...
                  <td className="px-3 py-3 text-right font-mono text-[12px] text-fg-default tabular-nums">{r.expense_ratio.toFixed(2)}</td>
                  <td className="px-3 py-3 text-right font-mono text-[12px] text-fg-default tabular-nums">{r.holdings_count}</td>
                  <td className="px-3 py-3 text-right">
                    <div className="inline-flex items-center gap-1.5 justify-end">
                      {r.deployable_cash === maxCash && (
                        <span className="font-mono text-[8.5px] tracking-meta uppercase text-white px-1 py-px rounded-sm" style={{ background: "var(--cat-1)" }}>
                          High
                        </span>
                      )}
                      <span className="font-mono text-[12px] text-fg-primary tabular-nums">{r.deployable_cash.toFixed(2)}</span>
                    </div>
                  </td>
                  <td className="px-3 py-3 text-right">
                    <div className="inline-flex items-center gap-1.5 justify-end">
                      {r.top10_concentration === maxConc && (
                        <span className="font-mono text-[8.5px] tracking-meta uppercase text-white px-1 py-px rounded-sm" style={{ background: "var(--cat-2)" }}>
                          Conc
                        </span>
                      )}
                      <span className="font-mono text-[12px] text-fg-default tabular-nums">{r.top10_concentration.toFixed(1)}</span>
                    </div>
                  </td>
                  <td className="px-3 py-3 text-right">
                    <button
